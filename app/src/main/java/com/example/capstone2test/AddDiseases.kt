...
import com.example.capstone2test.databinding.FragmentAddDiseasesBinding
...
class AddDiseases : Fragment() {
    ...
    override fun onCreateView(
        ...
        val autocompArray=resources.getStringArray(R.array.medList).toSet().toTypedArray()
        ...
        binding.medicationbottomNavView.setOnItemReselectedListener {
            ...
            when(item.itemId){
                R.id.homepage4 -> {
                    action=AddDiseasesDirections.actionGlobalHomepage()
                    Navigation.findNavController(binding.root).navigate(action)
                }
                R.id.settings4 -> {
                    action = AddDiseasesDirections.actionGlobalSettingsFrag()
                    Navigation.findNavController(binding.root).navigate(action)
                }
            }
            true
        ...
        binding.frommedicationaddFoodNow.setOnClickListener {
            action = AddDiseasesDirections.actionGlobalAddfood()
            Navigation.findNavController(binding.root)
        }
        binding.fromamedicationaddMedicationNow.setOnClickListener {

            action = AddDiseasesDirections.actionGlobalAddmed()
            Navigation.findNavController(binding.root)
        }
        binding.medicationscan.setOnClickListener {
            with(binding){
                if (ioViewModel.checkViewEmpty(medicationName,"Diseasese Name")
                    || ioViewModel.checkViewEmpty(subtype,"Knon Subtype")

                ){
                    return@setOnClickListener

                }
                //registerMed(medicationName.editText?.text.toString().trim(),medicationDailyGoal.editText?.text.toString().toInt() ,medicationDoseUnit.editText?.text.toString().trim(),medicaitonIllness.editText?.text.toString().trim())
